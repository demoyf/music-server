...
let demo = {
    ...
    _save_data(data,key_and_type,offset = 20) {
        ...
        for (let i = 0; i < arr.length; i += offset) {
            ...
            page++;
            _redis.saveBillBoardToRedis(key_and_type.key, page,JSON.stringify(obj));
        }
        let str_obj = {
    ...
    request_hot_artist() {
        ...
        _get_data.Net_getHotArtist().then((artist_list) => {
            if (artist_list) {
                this._save_into_redis(item.key, artist_list, item.page_num, { which_data: item.data_key_in_json, 
                    name: item.name });
            }
        });
    },
    _save_into_redis(key, data, page_num = 20, option) {
        ...
        let page = -1;
        for (let i = 0; i < arr.length; i += page_num) {
